...
export const createMedicalRecord = async (req: Request, res: Response): Promise<void> => {
  try {
    const { 
      ...
      diagnosis, 
      treatment_plan,
      actions, // array of { action_name, notes }
    ...
    const regResult = await pool.query('SELECT * FROM registrations WHERE id = $1', [registration_id]);
    ...
    try {
      ...
      if (actions && Array.isArray(actions) && actions.length > 0) {
        for (const action of actions) {
          await client.query(
            'INSERT INTO medical_actions (medical_record_id, action_name, notes) VALUES ($1, $2, $3)',
            [recordId, action.action_name, action.notes]
          );
        }
      }
      ...
      if (prescription_items && Array.isArray(prescription_items) && prescription_items.length > 0) {
        const presResult = await client.query(
          'INSERT INTO prescriptions (medical_record_id) VALUES ($1) RETURNING *',
          [recordId]
        );
        ...
        for (const item of prescription_items) {
          await client.query(
            'INSERT INTO prescription_items (prescription_id, medicine_name, dosage, quantity, instructions) VALUES ($1, $2, $3, $4, $5)',
            [presId, item.medicine_name, item.dosage, item.quantity, item.instructions]
          );
        }
      }
      ...
      return (successResponse(res, { medical_record_id: recordId }, 'Medical record created successfully', 201) as unknown) as void;
    } catch (e) {
